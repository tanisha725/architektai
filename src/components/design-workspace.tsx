...
// Gemini is the primary provider; Groq is the free fallback used when Gemini
// is unavailable (quota, rate limit, outage). Labeling both as "Gemini" would
// hide which one actually answered, so this is spelled out per-provider.
function providerLabel(provider: "gemini" | "groq" | null | undefined): string {
  if (provider === "groq") return "Groq (AI, fallback)";
  return "Gemini (AI)";
}
...
interface DesignWorkspaceProps {
  ...
  analysisProvider?: "gemini" | "groq" | null;
  ...
  architectureProvider?: "gemini" | "groq" | null;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          ...
          {architecture.domain && " · "}
          Generated by: {source === "ai" ? providerLabel(provider) : "rule-based fallback"}
        </p>
